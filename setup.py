...
def install():
    ...
    if ".solarbuddy" not in listdir(f"/home/{user_name}"):
        do(msg="create .solarbuddy directory",
            cmd=f'run_command("sudo mkdir /home/{user_name}/.solarbuddy/")') 
    do(msg="copy solarbuddy-config",
        cmd=f'run_command("sudo cp ./data/config /home/{user_name}/.solarbuddy/config")')
    do(msg="change directory owner",
        cmd=f'run_command("sudo chown -R {user_name}:{user_name} /home/{user_name}/.solarbuddy/")')

    print("Setup solarbuddy web-example service")
    do(msg="copy solarbuddy web-example file",
        cmd='run_command("sudo cp ./bin/solarbuddy-web-example /etc/init.d/solarbuddy-web-example")')
    do(msg="add excutable mode for solarbuddy-web-example",
        cmd='run_command("sudo chmod +x /etc/init.d/solarbuddy-web-example")')
...
setup(
    ...
    description='solarbuddy for Raspberry Pi',
    long_description=long_description,

    # The project's main homepage.
    url='https://github.com/jagadish-thimmasiddaiah/solarbuddy',

    # Author details
    author='Jagadish',
    author_email='tjagadish3@gmail.com',
    ...
    entry_points={
        'console_scripts': [
            'solarbuddy=psolarbuddy.utils:main', 
        ],
    },
)
